...
PCA dimensionality reduction + Platt-calibrated logistic regression.
"""
...
class AdaptiveDRE:
    """Density Ratio Estimator with PCA + Platt-scaled logistic regression.

    Pipeline:
        1. StandardScaler on combined source+target features
        2. PCA to ``n_components`` dimensions
    ...
    def fit(
        ...
        self._calibrated_clf.fit(X_pca, y)

        # Record domain AUC
        probs = self._calibrated_clf.predict_proba(X_pca)[:, 1]
    ...
    def compute_weights(self, features: np.ndarray) -> np.ndarray:
        ...
        # g(z) = P(target | z)
        g = self._calibrated_clf.predict_proba(X_pca)[:, 1]
